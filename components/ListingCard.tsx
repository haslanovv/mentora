...
export default function ListingCard({ listing, index = 0 }: { listing: any; index?: number }) {
  return (
    <motion.div initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.45, delay: Math.min(index * 0.05, 0.25) }} className="h-full">
    <Link href={`/listing/${listing.id}`} className="group block h-full">
      <div className="relative flex h-full min-h-[250px] flex-col justify-between overflow-hidden rounded-[24px] border border-[var(--line)] bg-[var(--surface-solid)] p-5 shadow-[0_8px_30px_rgba(46,38,29,0.04)] transition-all duration-500 hover:-translate-y-1.5 hover:shadow-[0_22px_45px_rgba(46,38,29,0.14)]">
        ...
        {listing.images?.[0] && <div className="relative -mx-5 -mt-5 mb-5 h-36 overflow-hidden bg-[var(--surface-muted)]"><img src={listing.images[0]} alt={`${listing.teacher_name} müəllim elanı`} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" /></div>}
        ...
        <div className="relative mb-6 space-y-3 text-sm text-[var(--ink-muted)]">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <svg className="h-4 w-4 text-[var(--accent)]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
            ...
            <span className="text-[11px] font-bold uppercase tracking-[0.1em]">
              {listing.format === 'online' ? 'Onlayn' : listing.format === 'eyani' ? 'Əyani' : 'Hər ikisi (Onlayn + Əyani)'}
            </span>
          </div>
        </div>
        ...
        <div className="relative mt-auto flex items-end justify-between border-t border-[var(--line)] pt-4">
          <div>
            <span className="mb-0.5 block text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--ink-muted)]">Qiymət</span>
